...
<x-site-layout title="เทียบโอนหน่วยกิต · นายทะเบียน">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <h1 class="mt-2 text-2xl font-bold">พิจารณาคำขอเทียบโอนหน่วยกิต</h1>
        <p class="text-gray-500 mt-1 text-sm">ตรวจสอบคำขอและอนุมัติหน่วยกิตเข้าคลังของผู้เรียน</p>
        ...
        <div class="mt-3 space-y-4">
            @include('partials.transfer-queue', ['pending' => $pending, 'approveRoute' => 'registrar.transfers.approve', 'rejectRoute' => 'registrar.transfers.reject'])
        </div>
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-gray-500 text-left">
                            <tr>
                                ...
                                <th class="px-5 py-3 font-medium">วิชา / ที่มา</th>
                                <th class="px-5 py-3 font-medium text-center">หน่วยกิต</th>
                                <th class="px-5 py-3 font-medium text-center">ผล</th>
                                <th class="px-5 py-3 font-medium text-right">โดย</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($reviewed as $t)
                                <tr>
                                    ...
                                    <td class="px-5 py-3">{{ $t->course_name }} <span class="text-gray-400">· {{ $t->source_name }}</span></td>
                                    <td class="px-5 py-3 text-center">{{ $t->status === 'approved' ? CreditRecord::fmt($t->credits_awarded) : '—' }}</td>
                                    ...
                                    <td class="px-5 py-3 text-right text-gray-500">{{ $t->reviewer?->name ?? '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
